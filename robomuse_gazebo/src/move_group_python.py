...
class MoveGroupPythonIntefaceTutorial(object):
  ...
  def define_positions(self):
    ...
    self.pose_drop = geometry_msgs.msg.Pose()
    p1x = 0.0855134703469
    p1y = -0.0122720720392
    p1z = 0.155374073824
    p1_ox = -0.018299603229
    p1_oy = -0.0058140490172
    p1_oz = -0.999616110566
    p1_ow = 0.019973804156
    self.pose_drop.position.x = p1x
    self.pose_drop.position.y = p1y
    self.pose_drop.position.z = p1z
    self.pose_drop.orientation.x = p1_ox
    self.pose_drop.orientation.y = p1_oy
    self.pose_drop.orientation.z = p1_oz
    self.pose_drop.orientation.w = p1_ow

    self.pose_inter = geometry_msgs.msg.Pose()
    p1x = 0.401831391641
    p1y = -0.311411135817
    p1z = 0.2516538851
    p1_ox = 0.00719426600046
    p1_oy = 0.00559607445013
    p1_oz = -0.648556623481
    p1_ow = 0.761111839762
    self.pose_inter.position.x = p1x
    self.pose_inter.position.y = p1y
    self.pose_inter.position.z = p1z
    self.pose_inter.orientation.x = p1_ox
    self.pose_inter.orientation.y = p1_oy
    self.pose_inter.orientation.z = p1_oz
    self.pose_inter.orientation.w = p1_ow
  ...
  def go_to_pose_goal(self, posint):
    ...
    if posint == 0:
    	...
    	pose_goal = self.pose_drop
    elif posint == 4:
    	pose_goal = self.pose_inter
  ...
  def go_to_joint_state(self, posint):
    group = self.group
  ...
  def plan_cartesian_path(self, dx, dy, dz):
    # Copy class variables to local variables to make the web tutorials more clear.
    # In practice, you should use the class variables directly unless you have a good
    # reason not to.
    group = self.group
    ...
    wpose.position.x += dx  # First move up (z)
    wpose.position.y += dy  # First move up (z)
    wpose.position.z += dz  # First move up (z)
...
def main():
  try:
    ...
    plan, fraction = moveGroup.plan_cartesian_path(0.1, 0, 0.0)
    print(fraction)
    if fraction==1.0:
        moveGroup.execute_plan(plan)
